...
function findAll(value, arr) {
	...
	return arr.filter(item => {
		...
		if (isEmptyOrWhiteSpace(item) || isEmptyOrWhiteSpace(value)) 
			return item === value;
		if (Number.isNaN(value) && Number.isNaN(item))
			return true;
		if (isNegativeZero(value))
			return isNegativeZero(item) || item === '-0';
		if (isNegativeZero(item))
			return isNegativeZero(value) || value === '-0';
		if (isNumberOrString(value) && isNumberOrString(item))
			return value == item;
